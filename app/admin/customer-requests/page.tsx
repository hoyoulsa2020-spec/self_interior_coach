...
export default function CustomerRequestsPage() {
  ...
  const fetchInquiries = useCallback(async () => {
    ...
    let query = supabase
      .from("inquiries")
      .select(`id, user_id, title, content, image_urls, status, answer, answered_at, created_at,
        profiles!inquiries_user_id_profiles_fkey(name, email, phone)`, { count: "exact" })
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-semibold text-gray-800">고객 문의 관리</h1>
        <p className="mt-0.5 text-sm text-gray-500">개인고객의 문의 내역을 확인하고 답변합니다.</p>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-sm md:hidden">
        {isLoading ? (
          ...
          <ul className="divide-y divide-gray-100">
            {inquiries.map((inq) => (
              <li key={inq.id} className="cursor-pointer px-4 py-3 active:bg-gray-50" onClick={() => openDetail(inq)}>
                <span className={`inline-block rounded-full px-2 py-0.5 text-[11px] font-medium ${inq.status === "answered" ? "bg-blue-50 text-blue-700" : "bg-yellow-50 text-yellow-700"}`}>
                  {inq.status === "answered" ? "답변완료" : "답변대기"}
                </span>
                <p className="mt-1 truncate font-medium text-gray-800">{inq.title}</p>
                <p className="mt-0.5 text-xs text-gray-500">{inq.profiles?.[0]?.name || "—"}</p>
                <button type="button" onClick={(e) => { e.stopPropagation(); openDetail(inq); }}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50 text-left text-xs font-semibold text-gray-500">
                  ...
                  <th className="hidden px-4 py-3 sm:table-cell">작성자</th>
